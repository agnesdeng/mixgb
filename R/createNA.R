...
#' @param cols  A vector specifying the names of the columns in which missing values should be generated.
#' @param p The proportion of missing values in the data frame or the proportions of missing values corresponding to the variables specified in \code{cols}.
...
#'   p = c(0.3, 0.2, 0.1)
#' )
createNA <- function(data, cols = NULL, p = 0.3) {
  ...
  if (is.null(cols)) {
    if (N.p == 1) {
      ...
      stop("When `cols` is not specified, the length of `p` should be either one or ncol(data).")
    }
  } else {
    Names <- colnames(data)
    ...
    if (N.p == 1) {
      ...
      stop("The length of `p` should be either one or the same as the length of `cols`.")
    }
