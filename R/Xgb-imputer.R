#' XGBoost imputer: Multiple imputation through XGBoost
#' @docType  class
#' @description Set up an XGBoost imputer object with specified hyperparameters and obtain multiply imputed datasets
#' @format  NULL
...
Mixgb <- R6Class("Mixgb",
                 cloneable = FALSE,
                 public = list(
                   #' @field data A data.frame or data.table with missing values.
                   #' @field xgb.params A list of XGBoost hyperparameters.
                   #' @field nrounds The maximum number of boosting iterations for XGBoost.
                   #' @field early_stopping_rounds An integer value \code{k}. XGBoost training will stop if the validation performance hasn't improved for \code{k} rounds.
                   #' @field print_every_n Print XGBoost evaluation information at every nth iteration if \code{verbose > 0}.
                   #' @field verbose Verbose setting for XGBoost training.
                   data = NULL,
                   xgb.params = NULL,
                   nrounds = NULL,
                   ...
                   #' @description Create a new \code{Mixgb} object. This is used to set up an XGBoost imputer with chosen hyperparameters and verbose options
                   ...
                   #' @description Impute missing values with the pre-set XGBoost imputer and obtain multiply imputed datasets, with an option to save models for imputing new data later on.
                   ...
                   impute = function(m = 5, maxit = 1, bootstrap = TRUE,pmm.type = "auto", pmm.k = 5, pmm.link = "prob", ordinalAsInteger = TRUE, initial.num = "normal", initial.int = "mode", initial.fac = "mode",save.models = FALSE, save.vars = NULL, ...) {
                     ...
                     # checking
                     check_pmm(pmm.type = pmm.type, bootstrap = bootstrap, xgb.params = self$xgb.params, Nrow = Nrow, sorted.naSums = sorted.naSums, sorted.types = sorted.types, pmm.k = pmm.k)
