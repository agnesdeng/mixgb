...
mixgb_load_use0 <- function(m.set, xgb.models, save.vars, save.p, extra.vars = NULL, extra.types = NULL, pmm.type, pmm.link, pmm.k, yobs.list, yhatobs.list = NULL,
  ...
  for (var in missing.vars) {
    ...
    if (missing.types[var] == "numeric" | missing.types[var] == "integer") {
      # xgb.models[[var]]<-xgb.load(xgb.models[[var]])
      # yhatmis <- predict(xgb.models[[var]], mis.data)
      yhatmis <- predict(xgb.load(xgb.models[[var]]), mis.data)
      if (!is.null(pmm.type)) {
      ...
      if (isFALSE(is_dir(xgb.models[[var]]))) {
        # load model
        yhatmis <- predict(xgb.load(xgb.models[[var]]), mis.data)
      ...
      if (is.null(pmm.type) | isTRUE(pmm.type == "auto")) {
        ...
        yhatmis <- predict(xgb.load(xgb.models[[var]]), mis.data, reshape = TRUE)
        if (pmm.type == 1) {
          # for pmm.type=1
