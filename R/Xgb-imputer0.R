#' Multiple imputation through xgboost R6 class imputer object (old version, just use for testing performance)
#' @docType  class
...
Mixgb0 <- R6Class("Mixgb0",
  ...
  public = list(
    ...
    impute = function(m = 5) {
      ...
      p <- ncol(data)
      if (p < 2) {
        stop("Data need to have more than two columns.")
      }
      Nrow <- nrow(data)
      ...
      sorted.df <- sortNA_df(data)$sorted.df
      sorted.idx <- sortNA_df(data)$sorted.idx
      ...
      Obs.index <- vector("list", p)
      Na.index <- vector("list", p)
          ...
          for (i in 1:p) {
            ...
            if (length(na.index) > 0) {
              ...
              if (type[i] == "numeric") {
                ...
                xgb.fit <- xgboost(
                  data = obs.data, label = obs.y, objective = obj.type, missing = NA, weight = NULL, nthread = self$nthread, early_stopping_rounds = self$early_stopping_rounds,
                ...
                xgb.fit <- xgboost(
                  data = obs.data, label = obs.y, objective = obj.type, num_class = N.class, missing = NA, weight = NULL, nthread = self$nthread, early_stopping_rounds = self$early_stopping_rounds,
                  nrounds = self$nrounds, max_depth = self$max_depth, gamma = self$gamma, eta = self$eta, colsample_bytree = self$colsample_bytree,
                  min_child_weight = self$min_child_weight, subsample = self$subsample, tree_method = self$tree_method, gpu_id = self$gpu_id, predictor = self$predictor, verbose = self$verbose, print_every_n = self$print_every_n
                )
                xgb.pred <- predict(xgb.fit, mis.data)
